...
class BrainTumorVGG16(nn.Module):
    def __init__(self, num_classes=1):  # Chỉ còn 1 output node để dùng BCEWithLogitsLoss
        ...
        self.vgg16.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(1024, num_classes)  # Không dùng Softmax
        )
...
model = BrainTumorVGG16(num_classes=1).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        epochs_without_improvement = 0
        torch.save(model.state_dict(), 'best_model_vgg16_v4.pth')
    else:
...
# Load the Best Model
model.load_state_dict(torch.load('best_model_vgg16_v4.pth'))
model.eval()
